import { BsPhone, BsEnvelopeAt, BsGithub, BsLinkedin } from 'react-icons/bs';
...
export default function Contact() {
  return (
    <h3 className="font-semi-bold flex flex-col">{contactNumbers.map(([icon, number], i) => <div className="flex items-center"><div className="w-4 m-2 inline-block">{icon}</div> {number}</div>)}</h3>
  );
}
